  ...
  options.modules.security.hardening = {
    enable = mkEnableOption "system security hardening" // {
      description = ''
        ...
        - Systemd service sandboxing and capability restrictions
    ...
    kernelSecurity = mkEnableOption "kernel security hardening" // {
      description = ''
        Enable kernel-level security hardening measures:
        - Memory initialization (init_on_alloc=1, init_on_free=1)
        - Kernel address space layout randomization
        - Slab allocator hardening (slab_nomerge)
    ...
    serviceSecurity = mkEnableOption "systemd service security hardening" // {
      description = ''
        Apply systemd service security restrictions:
        - System call filtering and architecture restrictions
        - Memory execution prevention (MemoryDenyWriteExecute)
        - Filesystem access restrictions (ProtectSystem, ProtectHome)
    ...
    auditingSecurity = mkEnableOption "security auditing and logging" // {
      description = ''
        ...
        - Enhanced systemd journal configuration with retention policies
