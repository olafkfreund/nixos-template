...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment = {
      ...
      # Configuration files
      etc = {
        # NOTE: hyprland.conf is no longer written here. The config lives in
        # home/profiles/hyprland.nix as structured
        # wayland.windowManager.hyprland.settings, so it is per-user and a host
        # can override one binding without restating the file.
