...
in
{
  options.modules.hardware.gpu.intel = {
    ...
    # Desktop options
    desktop = {
      enable = lib.mkEnableOption "desktop optimizations" // {
        default = isDesktop;
      };
      vaapi = lib.mkEnableOption "VA-API hardware acceleration" // {
        default = true;
      };
    ...
    compute = {
      enable = lib.mkEnableOption "compute optimizations" // {
        default = isCompute;
      };
    ...
    # Power management
    powerManagement = {
      enable = lib.mkEnableOption "Intel GPU power management" // {
        default = true;
      };
      rc6 = lib.mkEnableOption "RC6 power saving states" // {
        default = true;
      };
      fbc = lib.mkEnableOption "Frame Buffer Compression" // {
        default = true;
      };
      psr = lib.mkEnableOption "Panel Self Refresh" // {
        default = true;
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    hardware.graphics = {
      ...
      extraPackages32 = lib.mkIf isDesktop (
    ...
    environment.systemPackages =
      ...
      (
        ...
        ++
          lib.optionals
            (cfg.compute.enable && (cfg.generation == "arc" || cfg.generation == "xe") && cfg.compute.oneapi)
            [
    ...
    environment.sessionVariables = lib.mkMerge [
      ...
      (lib.mkIf isDesktop {
        ...
        # Vulkan
        VK_ICD_FILENAMES = lib.mkIf cfg.desktop.vulkan "/run/opengl-driver/share/vulkan/icd.d/intel_icd.x86_64.json";
      })
